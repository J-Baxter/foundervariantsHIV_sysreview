...
test <- meta::metaprop(multiplefounders,
               subjects,
               studlab = paste(publication),
               data = df_grouped,
               subset = NULL,
...
#pool effect size
meta_init <- MetaAll(df_grouped )
exposure.subgroup <- update.meta(test, byvar = reported.exposure, comb.random = TRUE, comb.fixed = FALSE)
